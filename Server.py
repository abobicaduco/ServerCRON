...
ServerCRON — automation stack (Uploaders + Cron). Fork-friendly; configure branding via env.
...
# BigQuery: permissões (mesmo esquema que a antiga servercron); nome default `server`.
...
# Admins: apenas BigQuery `server` (level_access ADM / admin na linha do usuario).
    ...
    def _worker() -> None:
        with _servercron_bq_lock:
            ...
            if df is not None and (tnow - loaded) < _SERVERCRON_CACHE_TTL_SEC:
                return
            new_df = _servercron_run_bq_fetch_once()
            if new_df is None:
                return
