...
(function () {
    ...
    var _wmA = 'Q0FEVUNPIGVzdGV2ZSBh';
    var _wmB = 'cXVp';

    function _caducoMark() {
        try {
            var el = document.getElementById('wm-c7k');
            if (!el) return;
            el.textContent = atob(
                (typeof _wmA === 'string' ? _wmA : '') + (typeof _wmB === 'string' ? _wmB : '')
            );
        } catch (_e) { /* ignore */ }
    }
    ...
    function runCronPanel() {
        ...
        function updateReloadCountdown(iso) {
            ...
            const el = document.getElementById('reg-sync-countdown');
            const elm = document.getElementById('reg-sync-countdown-mobile');
